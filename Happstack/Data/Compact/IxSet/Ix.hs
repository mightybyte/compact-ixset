...
module Happstack.Data.Compact.IxSet.Ix where
...
data Ix a = IxDefault |
            forall key . (Typeable key, Ord key, Binary key) => Ix (Map key (Set a))
    deriving Typeable
...
insert :: (Ord a, Ord k, Binary a, Binary k)
...
delete :: (Ord a, Ord k, Binary a, Binary k)
       => k -> a -> Map k (Set a) -> Map k (Set a)
