...
module Happstack.Data.Compact.IxSet (module Happstack.Data.Compact.IxSet,
                         module Ix)
    where

import qualified Happstack.Data.Compact.IxSet.Ix as Ix
import           Happstack.Data.Compact.IxSet.Ix (Ix(Ix))
import Data.Binary
...
instance (Serialize a, Ord a, Data a, Indexable a b,Binary a) => Serialize (IxSet a) where
    putCopy = contain . safePut . toList
    getCopy = contain $ liftM fromList safeGet

instance (SYBWC.Data ctx a, SYBWC.Sat (ctx (IxSet a)), SYBWC.Sat (ctx [a]),
          Indexable a b, Data a, Ord a,Binary a)
...
instance (Indexable a b, Data a, Ord a, Default a,Binary a) => Default (IxSet a) where
    defaultValue = ISet []

instance (Ord a,Show a,Binary a) => Show (IxSet a) where show = show . toSet

instance (Ord a,Read a,Data a,Indexable a b,Binary a) => Read (IxSet a) where
...
type IndexOp =
    forall k a. (Ord k,Ord a,Binary k,Binary a) => k -> a -> Map k (Set a) -> Map k (Set a)
...
change :: (Data a, Ord a,Data b,Indexable a b, Binary a) =>
...
insert :: (Data a, Ord a,Data b,Indexable a b, Binary a) => a -> IxSet a -> IxSet a
insert = change Ix.insert

-- | Removes an item from the IxSet
delete :: (Data a, Ord a,Data b,Indexable a b, Binary a) => a -> IxSet a -> IxSet a
...
updateIx :: (Indexable a b, Ord a, Data a, Typeable k, Binary k, Binary a)
         => k -> a -> IxSet a -> IxSet a
...
toSet :: (Ord a, Binary a) => IxSet a -> Set a
...
toSet' :: (Ord a, Binary a) => [Ix a] -> Set a
toSet' (Ix ix:_) = Map.fold Set.union Set.empty ix
toSet' [] = Set.empty
toSet' _ = error "unexpected match"

-- | Converts a Set to an IxSet
fromSet :: (Indexable a b, Ord a, Data a, Binary a) => Set a -> IxSet a
...
(&&&) :: (Ord a, Data a, Indexable a b, Binary a) => IxSet a -> IxSet a -> IxSet a
(&&&) = intersection
...
union :: (Ord a, Data a, Indexable a b, Binary a) => IxSet a -> IxSet a -> IxSet a
union x1 x2 = fromSet $ Set.union (toSet x1) (toSet x2)

-- | Takes the intersection of the two IxSets
intersection :: (Ord a, Data a, Indexable a b, Binary a) => IxSet a -> IxSet a -> IxSet a
...
(@=) :: (Indexable a b, Data a, Ord a, Typeable k, Binary k, Binary a)
    => IxSet a -> k -> IxSet a
ix @= v = getEQ v ix
...
getEQ :: (Indexable a b, Data a, Ord a, Typeable k, Binary k, Binary a)
      => k -> IxSet a -> IxSet a
getEQ = getOrd EQ
...
-- It is possible to provide a key of a type not indexed in the IxSet.  In
-- this case the function returns the empty IxSet for this type.
getLT :: (Indexable a b, Data a, Ord a, Typeable k, Binary k, Binary a)
      => k -> IxSet a -> IxSet a
getLT = getOrd LT
...
-- It is possible to provide a key of a type not indexed in the IxSet.  In
-- this case the function returns the empty IxSet for this type.
getGT :: (Indexable a b, Data a, Ord a, Typeable k, Binary k, Binary a)
      => k -> IxSet a -> IxSet a
getGT = getOrd GT
...
-- It is possible to provide a key of a type not indexed in the IxSet.  In
-- this case the function returns the empty IxSet for this type.
getLTE :: (Indexable a b, Data a, Ord a, Typeable k, Binary k, Binary a)
       => k -> IxSet a -> IxSet a
...
-- It is possible to provide a key of a type not indexed in the IxSet.  In
-- this case the function returns the empty IxSet for this type.
getGTE :: (Indexable a b, Data a, Ord a, Typeable k, Binary k, Binary a)
       => k -> IxSet a -> IxSet a
...
-- It is possible to provide a key of a type not indexed in the IxSet.  In
-- this case the function returns the empty IxSet for this type.
getRange :: (Indexable a b, Typeable k, Ord a, Data a, Binary k, Binary a)
         => k -> k -> IxSet a -> IxSet a
...
-- inference.
groupBy::(Typeable k,Typeable t,Ord t,Binary k,Binary t) =>  IxSet t -> [(k, [t])]
...
-- | A reversed groupBy
rGroupBy :: (Typeable k, Typeable t,Ord t,Binary k,Binary t) => IxSet t -> [(k, [t])]
rGroupBy = reverse . groupBy
 
splitLookup :: forall k a.  (Ord k, Binary k, Binary a)
            => k -> Map k a -> (Map k a, Maybe a, Map k a)
splitLookup k m = (smaller, eq, larger)
  where (smaller, gte) = Map.partitionWithKey (\k' _ -> k' < k) m
        (_, larger) = Map.partitionWithKey (\k' _ -> k' == k) gte
        eq = Map.lookup k m

--query impl function
-- | A function for building up selectors on IxSets.  Used in the various get*
-- functions. 
getOrd :: (Indexable a b, Ord a, Data a, Typeable k, Binary k, Binary a)
...
instance (Show a,Indexable a b,Data a,Ord a, Binary a) => Monoid (IxSet a) where
    mempty=empty
    mappend = union
